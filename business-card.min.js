class r extends HTMLElement {
  ...
  attributeChangedCallback(t, i, s) {
    const e = t.replace(/-([a-z])/g, (o) => o[1].toUpperCase());
    this[e] = s, this.render();
  }
  connectedCallback() {
    this.render();
  }
...
EMAIL:${this.email || ""}
END:VCARD`;
    return `<img src="https://api.qrserver.com/v1/create-qr-code/?size=72x72&data=${encodeURIComponent(t)}"/>`;
  }
  render() {
    const t = this.variant || "wine", i = this.position;
    if (this.shadowRoot.innerHTML = `
      ...
      ${this.floating ? `
        ...
        <div class="wrapper pos-${i}" id="wrapper">
          ${this.cardHtml(t)}
        </div>
      ` : this.cardHtml(t)}
    `, this.floating) {
      const s = this.shadowRoot.getElementById("fab"), e = this.shadowRoot.getElementById("wrapper");
      s.onclick = () => e.classList.toggle("open"), document.addEventListener("click", (o) => {
        this.contains(o.target) || e.classList.remove("open");
      });
    }
  }
  cardHtml(t) {
...
customElements.define("business-card", r);
